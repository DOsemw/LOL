...
def add_player_rolling_features(df: pd.DataFrame) -> pd.DataFrame:
    ...
    df["player_kda_roll5"] = df.groupby(["playername", "position"], group_keys=False).apply(
        lambda g: g.rolling(5, min_periods=1).apply(
            lambda x: x[-1] if len(x) else np.nan
        ).shift(1)["kills"]  # placeholder — real kda done below
    )

    # Simpler: precompute then roll
