...
def predict_player(
    ...
    # Apply moneyline adjustment multipliers inline
    if moneyline is not None and opp_ml is not None:
        from moneyline_adjustments import _interpolate_adjustment
        for stat in ["kills", "deaths", "assists"]:
            mult = _interpolate_adjustment(win_prob, stat)
            pg[stat]["per_game"] = round(pg[stat]["per_game"] * mult, 2)
            pg[stat]["low"]      = round(pg[stat]["low"]      * mult, 2)
            pg[stat]["high"]     = round(pg[stat]["high"]     * mult, 2)
