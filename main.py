...
def load_everything():
    ...
    # Compute data-driven kill bounds per position (5th/95th percentile)
    kill_bounds = {}
    for pos in ["top", "jng", "mid", "bot", "sup"]:
        pos_kills = feat[feat["position"] == pos]["kills"]
        if len(pos_kills) >= 20:
            kill_bounds[pos] = {
                "p5":  round(float(pos_kills.quantile(0.05)), 2),
                "p95": round(float(pos_kills.quantile(0.95)), 2),
                "mean": round(float(pos_kills.mean()), 2),
            }
    log.info(f"Kill bounds: {kill_bounds}")
...
@app.get("/stats")
def get_stats():
    """Kill bounds and pace multipliers computed from your 2026 data."""
    _check_ready()
    return {
        "kill_bounds":      STATE.get("kill_bounds", {}),
        "pace_multipliers": STATE.get("pace_multipliers", {}),
