...
def predict_player(df, tk_model, tk_features, ks_models, ks_features,
    ...
    # Clamp predictions to data-driven bounds (p5 to p95 per position)
    if kill_bounds and position in kill_bounds:
        bounds = kill_bounds[position]
        kills_per_map = float(np.clip(kills_per_map, bounds["p5"], bounds["p95"]))
